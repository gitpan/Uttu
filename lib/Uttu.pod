...
This is a string denoting the database, etc., hosting the
...
This is the DBI driver to use.  C<DBD::$global_db_driver> (and
C<DBD::$global_db_const_driver>) must be loadable.
...
=item db_host, db_const_host

This is the host where the database is located.

If multiple C<db_host> lines are present, Uttu will cycle through them
until it exhausts all available hostnames, or it finds one that
allows a connection.  This allows failover to alternate hosts.

C<db_host> should allow modifications as well as read-only access.
Uttu itself only neads read access.  C<db_const_host> implies that the
database will not be modified.
...
This will return a database handle (result of a C<DBIx::Abstract-E<gt>connect> call)
...
If this option is true, then the "global_db_*" configuration settings are
not used if the equivalent settings for the requested database coniguration
cannot be found.

=item Write

If this option is true, then the C<_const> part of the prefix will not be
considered.  This is useful for specifying a database for reading and
another for writing, with round robin access to either as desired.

=back

=item query_dbh_info

This method takes the same arguments as C<query_dbh> and returns 
a reference to a hash containing the paramters used to connect to
the database.  The hash has the following keys.
...
This is an array reference to one or more hostnames.  The first 
hostname in the array is the current (or most recent) host being used.
...
This is a reference to a hash of DBI options.

=back 4

=item query_host_available

Returns a true value if the availability of the host is being 
tracked and the host is marked as available, or if the host is not 
being tracked.  If the host is tracked and marked unavailable, 
returns false.

 if($u -> query_host_available('localhost')) {
...
=item set_host_available

Marks the host as available.  If the optional second argument is 
given, it will set or unset the availability of the host.

 $u -> set_host_available('localhost');

or

 $u -> set_host_available('localhost', 1);

=item set_host_unavailable

Marks the host as unavailable.  If the optional second argument 
is given, it will set or unset the unavailability of the host.

 $u -> set_host_unavailable('localhost');

or

 $u -> set_host_unavailable('localhost', 1);
...
=head2 Database connections disappear

For some reason, if a website is left idle for a long time (on the 
order of days), the uri-to-filename mapping code will be unable to 
read the database (it seems). This will result in pages not being 
found on the server.  The fix for now is to stop and restart the 
web server.  This may be fixed already.  It has not shown up for a 
while in sites run by the author.

We may move to using ResourcePool to manage DBI connections, in 
part to fix this.
...
$Id: Uttu.pod,v 1.13 2003/03/12 06:33:48 jgsmith Exp $
